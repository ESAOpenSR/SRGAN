...
class Sentinel2TestDataSet(torch.utils.data.Dataset):
    ...
    def __init__(self, data_folders=None, amount=100, band_selection="R10m"):
        ...
        if type(data_folders) == list:
            self.amount = int(
                self.amount / len(data_folders)
            )  # divide by number of folders to keep total amount the same
    ...
    def get_windows(self, data_folder):
        ...
        if self.band_selection == "R10m":
            image_files = {
                "R": os.path.join(
                    folder_path, [file for file in file_paths if "B04" in file][0]
                ),
                "G": os.path.join(
                    folder_path, [file for file in file_paths if "B03" in file][0]
                ),
                "B": os.path.join(
                    folder_path, [file for file in file_paths if "B02" in file][0]
                ),
                "NIR": os.path.join(
                    folder_path, [file for file in file_paths if "B08" in file][0]
                ),
            }
        if self.band_selection == "R20m":
            image_files = {
                "B05": os.path.join(
                    folder_path, [file for file in file_paths if "B05" in file][0]
                ),
                "B06": os.path.join(
                    folder_path, [file for file in file_paths if "B06" in file][0]
                ),
                "B07": os.path.join(
                    folder_path, [file for file in file_paths if "B07" in file][0]
                ),
                "B8A": os.path.join(
                    folder_path, [file for file in file_paths if "B8A" in file][0]
                ),
                "B11": os.path.join(
                    folder_path, [file for file in file_paths if "B11" in file][0]
                ),
                "B12": os.path.join(
                    folder_path, [file for file in file_paths if "B12" in file][0]
                ),
            }
